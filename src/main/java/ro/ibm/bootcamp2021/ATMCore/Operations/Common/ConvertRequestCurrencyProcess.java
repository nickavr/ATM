    ...
    public void handleRequest(InputRequest request, Account currentAccount) {
        if(request != null){
            ...
            CommonFunctionalitiesInterface.setATMNewDeposit(request.getCurrencyType(),
                    request.getAmount(), request.getOption());

            request.setAmount(chooseExchangeRate(currentAccount.getAccountCurrency(),
                    request.getCurrencyType()).convertAmount(request.getAmount()));
